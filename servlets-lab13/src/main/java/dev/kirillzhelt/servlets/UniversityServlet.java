...
public class UniversityServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String relativeURL = request.getRequestURI()
            .replaceFirst("/servlets_lab13_war_exploded/UniversityServlet", "");

        Command command = CommandManager.getCommandForURL(relativeURL);
        if (command instanceof MainCommand) {
            ((MainCommand) command).executeGet(new CommandParams(request, response));
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (command instanceof PostCommand) {
            ((PostCommand) command).executePost(RegisterEnrolleeParams.fromPOSTParams(request, response));
        }
    }
